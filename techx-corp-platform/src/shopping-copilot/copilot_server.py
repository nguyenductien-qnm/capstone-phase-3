...
class ShoppingCopilotServicer(pb_grpc.ShoppingCopilotServiceServicer):
    ...
    def ChatWithCopilot(self, request, context):
        ...
        # Route FIRST so cache key is scoped to the actual model variant.
        # Using MAIN_MODEL (env default) here would cause Nova Pro responses to be cached
        # under a Nova Lite key, serving Lite-cohort users answers generated by Pro — a
        # fundamental A/B integrity violation.
        ...
        # Use the ROUTED model id (not MAIN_MODEL env default) so Lite and Pro caches
        # never collide.  Strip the ARN prefix if present for readability.
        _public_routed = routed_model.rsplit("/", 1)[-1] if routed_model.startswith("arn:") else routed_model
        model_ver = _public_routed.replace(":", "-")
        prompt_ver = CODE_FP
