...
1. **Chuyển dịch sang AWS Bedrock:** `shopping-copilot` sử dụng SDK boto3 để gọi trực tiếp Amazon Bedrock thay vì gọi Mock LLM (`http://llm:8000/v1`). 
   ...
   - **Bảo mật Cross-account:** Để AssumeRole thành công, bắt buộc phải cấu hình `BEDROCK_AWS_EXTERNAL_ID=phase3-bedrock-cross-account` vào biến môi trường của container.
   ...
   - **Bedrock Guardrails:** Hệ thống hỗ trợ sẵn Guardrail. Không yêu cầu CDO dùng Terraform tạo mới, chỉ cần lấy ID Guardrail đã có và map vào biến `BEDROCK_GUARDRAIL_ID`.
...
**Nova judge**. Eval 25/25 pass (cập nhật 20/07, mở rộng từ 18 case — script đổi tên
thành `docs/ai/evals/eval_mandate06_v6.py`), report: `docs/ai/evals/eval_mandate06_v6_report.md`.
...
- Shopping Copilot chạy **`us-east-1`**. Ngày 22/07/2026, profile SSO CDO
  `Phase3-CDO-PermissionSet-804372444787` đã invoke trực tiếp Nova Pro/Lite thành công trong
  account `804372444787`; lỗi `OperationNotAllowed` trước đây không còn tái hiện.
- Pod không dùng SSO local: `shopping-copilot` tiếp tục dùng EKS Pod Identity rồi assume role
  Bedrock cross-account. Vì vậy phải verify in-pod sau deploy; kết quả SSO chỉ xác nhận model
  access và phép đo local, không thay thế quyền runtime của pod.
