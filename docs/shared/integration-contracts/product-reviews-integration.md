...
|---|---|---|
| `AWS_REGION` / `AWS_BEDROCK_MODEL` | `us-east-1` / `amazon.nova-lite-v1:0` | Bedrock region/model; credentials come from IRSA/Pod Identity |
| `BEDROCK_AWS_ROLE_ARN` / `BEDROCK_AWS_EXTERNAL_ID` | unset | optional cross-account Bedrock role; pod IRSA role must be allowed to `sts:AssumeRole` |
