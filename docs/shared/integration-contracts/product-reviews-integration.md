...
|---|---|---|
| `AWS_REGION` / `AWS_BEDROCK_MODEL` | `us-east-1` / `amazon.nova-lite-v1:0` | EKS Pod Identity + cross-account role đã có trong Terraform; cần verify in-pod sau deploy |
| `BEDROCK_AWS_ROLE_ARN` / `BEDROCK_AWS_EXTERNAL_ID` | unset / `phase3-bedrock-cross-account` | optional cross-account Bedrock role; pod IRSA role phải được phép `sts:AssumeRole`; External ID **bắt buộc** truyền để AssumeRole thành công trong kịch bản SSO Cross-account |
| `BEDROCK_GUARDRAIL_ID` | (không bắt buộc) | Cấu hình ID của Guardrail nếu dùng, ứng dụng hỗ trợ sẵn, không yêu cầu Terraform tạo mới |
| `LLM_REVIEWS_MAIN_MODEL` / `LLM_REVIEWS_FALLBACK_MODEL` | nova-lite / nova-micro | routing |
| `LLM_REVIEWS_TIMEOUT` / `LLM_REVIEWS_FALLBACK_TIMEOUT` | 2.6 / 2.3 | giây, theo P95 đo thật 2026-07-21 |
...
## Phụ lục 14/07/2026 — Cập nhật Model Gateway & A/B Testing (Đua Top)

Các thay đổi mới nhất áp dụng cho hệ thống:

1. **Routing qua Model Gateway:** `product-reviews` hiện tại tự động lấy danh sách và trọng số model từ cờ `llmModelRouting` của `flagd`. Các biến môi trường `LLM_REVIEWS_MAIN_MODEL` sẽ đóng vai trò dự phòng (fallback) nếu flagd lỗi.
2. **Flagd mới cần thêm:**
   - Cờ `llmModelRouting` (kiểu JSON Object / Fractional Evaluation) xác định tỷ lệ % traffic vào các model (VD: 80% Nova-Lite, 20% Nova-Pro).
3. **Telemetry Metrics:** Đo metric chia theo `model_id` thay vì static label. Dashboard Grafana phía CDO cần filter theo biến `model_id` để phân biệt cost/latency.
