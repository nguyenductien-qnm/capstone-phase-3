...
|---|---|---|
| `AWS_REGION` / `AWS_BEDROCK_MODEL` | `us-east-1` / `amazon.nova-lite-v1:0` | Bedrock region/model; uses `BEDROCK_AWS_*` Account 2 credentials when present, otherwise Pod Identity/IAM |
| `BEDROCK_AWS_ACCESS_KEY_ID` / `BEDROCK_AWS_SECRET_ACCESS_KEY` | unset | optional dedicated Bedrock Account 2 credentials from Kubernetes secret `bedrock-credentials` |
