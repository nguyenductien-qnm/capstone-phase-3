...
| S5 | Copilot 3 intent + confirmation gate | Eval `golden_qa_dataset.json` 24 case trên agent thật | ✅ đã code xong trên nhánh `feat/TF1-57-59-68` |
| S6 | Không kéo SLO storefront xuống | So metric trước/sau trên EKS | ⏳ cần cluster |

## 4. Out of scope tuần 1
Auto-remediation (detect-only), multi-region. (Semantic search bằng `pgvector` đã được đưa vào luồng triển khai).
...
| Đề yêu cầu | Docs | Code | Trạng thái |
|---|---|---|---|
| 3 intent + confirmation gate | `03_specs/shopping_copilot.md` + contract :50051 + `evals/golden_qa_dataset.json` (24 case) | Đã hoàn thiện code trên `feat/TF1-57-59-68` | ✅ **Implemented** (có Action Gate theo MANDATE-06) |
...
### Kết luận phủ đề
Sau các fix 12/07 tối và hoàn thiện tính năng tuần 2: **Phần A phủ đủ 100% yêu cầu chữ-đen-trắng của đề** (guardrail là mảnh cuối, vừa đóng); **Phần B (Shopping Copilot) đã triển khai xong code thật kèm Action Gate (MANDATE-06)**; **Semantic Search với `pgvector` đã được tích hợp thành công**. AIOps core phủ 5/6 nhóm tín hiệu (cost = gap có kế hoạch); mở rộng ở mức backlog có phương pháp. Mọi trạng thái ⏳/draft đều có script/kế hoạch đo kèm hoặc đã xử lý xong.
