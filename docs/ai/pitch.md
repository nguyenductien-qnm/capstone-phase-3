...
* **Lập luận bảo vệ:**
  ...
  >
  > ***Đã có trong code hôm nay:*** *`product-reviews` gọi Amazon Bedrock Nova Lite qua Converse API, có Valkey cache với versioned key và TTL động 4h–7d, và detector cảnh báo vận hành (`aiops/detector`) đã chạy với 9 rule bám đúng ngưỡng SLO thật (error rate 0.5%, checkout 1%, p95 1.0s).*
  >
