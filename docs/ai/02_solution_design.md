...
observability: OTel → Prometheus/OpenSearch/Jaeger ──► aiops-detector (poll 30s, 11 rule) ──► webhook alert
...
|---|---|---|
| Model routing | Nova Lite→Micro (reviews), Nova Pro→Lite (copilot) | Claude (chốt bỏ 11/07; đắt ~50×); single-model (không có fallback) |
...
| Tìm sản phẩm NL | Catalog-in-prompt (10 sản phẩm ≈ vài trăm token) | pgvector/HNSW (over-engineer ở N=10 — defer, trigger: catalog >~500) |
