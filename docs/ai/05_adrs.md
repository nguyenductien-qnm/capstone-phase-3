...
**Status:** Implemented
**Date:** 2026-07-14
**Context:** Tính năng AI (như tóm tắt review, shopping copilot) hiển thị trực tiếp cho khách hàng. Cần đảm bảo hệ thống chặn được Prompt Injection, không bị lộ thông tin cá nhân (PII), chống ảo giác (Hallucination) và fallback an toàn khi gặp sự cố, đáp ứng yêu cầu của MANDATE-06 (trụ AIE).

## Yêu cầu cốt lõi
1. **Fallback an toàn**: Chạy LLM thật, có chiến lược fallback khi timeout, rate limit.
2. **Không Hallucination**: AI không được trả lời các thông tin ngoài lĩnh vực (ví dụ: "thủ đô nước Pháp").
3. **Chống Prompt Injection & PII**: AI không bị dắt mũi bởi lệnh điều khiển trong user input hoặc trong nội dung review (data poisoning).
4. **Action Gate**: Không tự ý checkout, phải hỏi xin phép người dùng.
5. **Đánh giá (Eval)**: Mentor phải tự bắn thử và kiểm chứng qua script hoặc UI.

## Quyết định (Decision)
1. **Áp dụng Defense-in-Depth Guardrails**:
   - Tầng 1: **Input/Output Regex Filtering**: Tái sử dụng `guardrails.py` cho `shopping-copilot` để chặn keyword injection (VD: "ignore previous instructions") và lọc PII (email, phone number) trước khi đưa prompt tới LLM.
   - Tầng 2: **System Prompt Engineering**: Hướng dẫn LLM bỏ qua các thông tin rác, chỉ tập trung vào nghiệp vụ catalog (Shopping Copilot) và không tiết lộ hướng dẫn của hệ thống.
   - Tầng 3: **Output Checking**: Bắt buộc kiểm tra kết quả trả về, nếu kết quả chứa >40 ký tự trùng với system prompt -> chặn và trả fallback "Hệ thống quá tải".
2. **Xác nhận hành động (Confirmation Gate)**:
   - Tool `add_item_to_cart` trong Shopping Copilot chỉ tạo `PendingAction`. Server phải trả về `confirmation_token`, và khách hàng (client) gửi lại token này ở request thứ 2 để thực thi hành động, bypass LLM. Chặn đứng rủi ro AI tự ý thay đổi giỏ hàng.
3. **Fallback & Resiliency**: 
   - Đã áp dụng Bulkhead, Circuit Breaker và tự động Fallback sang model nhẹ hơn (Nova Micro) khi Nova Pro/Lite gặp sự cố. (Kế thừa từ ADR-010).

## Đánh giá (Evaluation)
- **Script Eval**: Viết sẵn script `src/shopping-copilot/eval_mandate06.py` nhằm tự động giả lập 3 kịch bản:
  - Test 1: Prompt Injection ở user input.
  - Test 2: Hallucination (câu hỏi ngoài lề).
  - Test 3: Ép AI tự thanh toán / add vào cart.
- Mentor có thể chạy script để quan sát hành vi bị chặn/fallback một cách tái tạo được (reproducible).
