...
### 2c — Karpenter NodePool có zone minValues:2

> ⚠️ **PRE-CHECK bắt buộc:** manifest `develop-karpenter-nodepool` CÓ trong Git
> (`platform/gitops/environments/develop/applications/karpenter-nodepool.yaml`, path `.../develop/karpenter`),
> **nhưng lần kiểm gần nhất KHÔNG thấy app này trên cluster develop** (chỉ có develop-root,
> develop-techx-corp, develop-external-secrets, develop-aws-load-balancer-controller,
> develop-metrics-server, develop-frontend-proxy-nlb) ⇒ **Karpenter có thể chưa cài ở develop**.

```bash
# PRE-CHECK 1: app karpenter có tồn tại không?
kubectl --context "$DV" -n argocd get application develop-karpenter-nodepool 2>&1

# PRE-CHECK 2: CRD NodePool + controller karpenter có không?
kubectl --context "$DV" get crd nodepools.karpenter.sh 2>&1
kubectl --context "$DV" -n kube-system get deploy karpenter 2>&1
```

**Nếu PRE-CHECK fail (chưa có Karpenter ở develop):**
- **Bỏ qua 2c/2d ở develop**, verify R2-Karpenter ở **sandbox** (nơi Karpenter đang chạy), HOẶC
- Sync `develop-karpenter-*` apps trước (cần develop-root sync) rồi mới test.
- Ghi rõ lựa chọn này vào evidence để mentor biết.

**Nếu PRE-CHECK pass:**
```bash
kubectl --context "$DV" -n argocd patch application develop-karpenter-nodepool \
  --type=merge -p '{"operation":{"sync":{}}}'

kubectl --context "$DV" get nodepool default -o yaml | grep -E "minValues|zone"
```

**✅ Pass khi:** Thấy `minValues: 2`.

---

### 2d — Drain test (mô phỏng mất 1 AZ)

> ⚠️ Chỉ làm khi 2a/2b/2c xanh. Báo team trước.

```bash
NODE=$(kubectl --context "$DV" -n $NS get pod -l opentelemetry.io/name=payment \
  -o jsonpath='{.items[0].spec.nodeName}')

echo "Draining: $NODE"

kubectl --context "$DV" drain $NODE \
  --ignore-daemonsets --delete-emptydir-data --timeout=120s

# Verify payment luôn có ≥1 Running
kubectl --context "$DV" -n $NS get pod -l opentelemetry.io/name=payment -w
```

**✅ Pass khi:** payment không bao giờ = 0 pod Running trong lúc drain.

**⚠️ Blocker:** PDB block (replicas=1) → drain treo. Fix: scale lên 2 trước.
