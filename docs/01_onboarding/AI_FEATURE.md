...
- `llm` mặc định là **mock**. Cắm model thật (gpt-4o-mini / Bedrock…) qua `platform/gitops/environments/sandbox/values-aio-llm.yaml` + secret `llm-api-key` (xem GETTING_STARTED).
